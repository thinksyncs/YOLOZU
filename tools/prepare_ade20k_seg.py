...
from yolozu.datasets.ade20k import (  # noqa: E402
    ADE20K_IGNORE_INDEX,
    iter_ade20k_samples,
    load_ade20k_classes,
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Prepare ADE20K semantic segmentation layout + dataset.json.")
    p.add_argument("--ade20k-root", required=True, help="ADE20K root (or a parent dir containing ADEChallengeData2016).")
    ...
    p.add_argument(
        "--ignore-index",
        type=int,
        default=int(ADE20K_IGNORE_INDEX),
        help="Ignore index value for masks (metadata only; default: 255).",
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    resolved = resolve_ade20k_paths(args.ade20k_root)
    ...
    samples = list(iter_ade20k_samples(resolved.root, split=split))
    ...
    classes = load_ade20k_classes(resolved.root)

    payload = {
        "dataset": "ade20k",
        "task": "semantic_segmentation",
        "split": split,
        "source_root": str(resolved.root),
        "mode": mode,
