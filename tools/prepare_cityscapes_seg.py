...
from yolozu.datasets.cityscapes import (
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Prepare a Cityscapes semantic segmentation dataset layout + dataset.json.")
    p.add_argument("--cityscapes-root", required=True, help="Cityscapes root directory.")
    p.add_argument("--split", choices=("train", "val", "test"), default="train", help="Split to prepare.")
    p.add_argument(
        "--label-type",
        choices=("labelTrainIds", "labelIds"),
        default="labelTrainIds",
        help="Which Cityscapes label image to use.",
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    mode = str(args.mode)
    if mode != "manifest":
        _ensure_empty(out_root / "images" / str(args.split), force=bool(args.force))
        _ensure_empty(out_root / "masks" / str(args.split), force=bool(args.force))

    samples = list(iter_cityscapes_samples(args.cityscapes_root, split=str(args.split), label_type=str(args.label_type)))
