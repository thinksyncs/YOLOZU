...
\ChapterMeta{
Cover parity checking (backend drift detection), protocol-pinned evaluation settings, and benchmark/sweep harnesses for regression surfaces.
}{
Makes comparisons stable and actionable by detecting silent numeric drift and tracking performance/metrics trends over time.
}{
Requires at least two comparable prediction artifacts for parity; protocol runs require pinned settings files and consistent preprocessing/output formats.
}
...
\path{reports/hpo\_sweep.jsonl}, \path{reports/hpo\_sweep.csv}, \path{reports/hpo\_sweep.md}.
