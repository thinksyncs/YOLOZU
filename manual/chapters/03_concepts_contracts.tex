...
\ChapterMeta{
Explain the core contracts (schemas) and the dataset/predictions migration approach used throughout the repository.
}{
Keeps evaluation stable across backends and over time by making inputs/outputs explicit and versionable.
}{
Assumes you follow the documented schemas and treat generated wrapper artifacts (e.g., \path{dataset.json}) as part of the workflow.
}
...
\begin{itemize}
  ...
  \item output: per-image \cmd{detections} with \cmd{class\_id}, \cmd{score}, \cmd{bbox} (typically \cmd{cxcywh\_norm}), plus task-specific optional fields,
