...
\ChapterMeta{
Provide canonical workflows to (A) evaluate existing artifacts, (B) export predictions from a backend, and (C) run convenient folder inference.
}{
Standardizes how \path{predictions.json} and reports are produced so results are comparable and automatable.
}{
You need a dataset root and one or more predictions artifacts; backend-specific steps may require extra dependencies (Torch/ONNX Runtime/TensorRT).
}
