...
\ChapterMeta{
Explain the training stack focus (RT-DETR pose scaffold) and the run contract: stable artifact layout, strict config resolution, and standardized resume.
}{
Enables reproducibility, parity checks, and consistent comparisons across machines/branches by making run outputs predictable.
}{
Requires using run-contract mode (e.g., \cmd{--run-id} / \cmd{--run-contract}) and respecting strict config keys; training dependencies apply.
}
...
model JSON (\texttt{model.*}) & \texttt{stem\_channels} & stem width \\
model JSON (\texttt{model.*}) & \texttt{backbone\_channels} & per-stage channels \\
model JSON (\texttt{model.*}) & \texttt{stage\_blocks} & per-stage block depth \\
model JSON (\texttt{model.*}) & \texttt{backbone\_activation} & backbone activation preset \\
model JSON (\texttt{model.*}) & \texttt{backbone\_kwargs} & optional backbone-specific kwargs \\
