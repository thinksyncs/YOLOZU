...
\begin{itemize}
  ...
  \item Fixed artifact layout under \path{runs/<run_id>/}:
    \begin{itemize}
  ...
  \item Export/parity artifacts are produced in contracted runs by default:\
    ...
    \path{reports/onnx_parity.json}.
  \item Full-state resume is standardized with \cmd{--resume} from
    \path{runs/<run_id>/checkpoints/last.pt}.
\end{itemize}
...
The trainer supports optional depth integration with a safe default:
\begin{itemize}
  \item \cmd{--depth-mode none} (default): baseline path, depth disabled.
  \item \cmd{--depth-mode sidecar}: reads per-image depth sidecars (\cmd{depth_path}/\cmd{depth}) and propagates \cmd{depth_valid}.
  \item \cmd{--depth-mode fuse_mid}: applies lightweight depth fusion after projector (outside the backbone boundary).
\end{itemize}

Safety knobs:
\begin{itemize}
  \item \cmd{--depth-unit \{unspecified,relative,metric\}} (default: \cmd{unspecified}).
  \item Absolute depth matcher costs are allowed only in metric mode; non-metric modes safety-disable \cmd{cost_z}/\cmd{cost_t}.
  \item \cmd{--depth-scale} applies a scalar to sidecar depth values.
  \item \cmd{--depth-dropout} controls modality dropout probability for \cmd{fuse_mid} training.
\end{itemize}
