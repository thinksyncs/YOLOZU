...
\begin{itemize}
  ...
  \item \cmd{--depth-mode sidecar}: reads per-image depth sidecars (\cmd{depth\_path}/\cmd{depth}) and propagates \cmd{depth\_valid}.
  \item \cmd{--depth-mode fuse\_mid}: applies lightweight depth fusion after projector (outside the backbone boundary).
\end{itemize}

Safety knobs:
\begin{itemize}
  \item \cmd{--depth-unit \{unspecified,relative,metric\}} (default: \cmd{unspecified}).
  \item Absolute depth matcher costs are allowed only in metric mode; non-metric modes safety-disable \cmd{cost\_z}/\cmd{cost\_t}.
  \item \cmd{--depth-scale} applies a scalar to sidecar depth values.
  \item \cmd{--depth-dropout} controls modality dropout probability for \cmd{fuse\_mid} training.
\end{itemize}
