...
\midrule
Train (scaffold) & \path{configs/examples/train\_setting.yaml} & \cmd{yolozu train configs/examples/train\_setting.yaml} \\
Train (contract) & \path{configs/examples/train\_contract.yaml} & \cmd{yolozu train configs/examples/train\_contract.yaml --run-id exp01} \\
Resume (full state) & same as contract & \cmd{yolozu train configs/examples/train\_contract.yaml --run-id exp01 --resume} \\
Test (scenario runner) & \path{configs/examples/test\_setting.yaml} & \cmd{yolozu test configs/examples/test\_setting.yaml --adapter precomputed --predictions reports/predictions.json} \\
\bottomrule
\end{longtable}
...
model JSON (\texttt{model.*}) & \texttt{backbone\_name} & backbone family (default: \texttt{cspresnet}) \\
model JSON (\texttt{model.*}) & \texttt{stem\_channels}, \texttt{backbone\_channels}, \texttt{stage\_blocks} & width/depth structure \\
model JSON (\texttt{model.*}) & \texttt{backbone\_activation}, \texttt{backbone\_kwargs} & activation and optional backbone args \\
solver YAML/CLI & \texttt{backbone\_lr\_mult}, \texttt{backbone\_wd\_mult} & optimization scaling for backbone params \\
\bottomrule
\end{longtable}
...
\begin{enumerate}
  ...
  \item Edit \texttt{model.backbone\_name} and related channel/block fields.
...
# edit model.backbone_name / backbone_channels / stage_blocks
