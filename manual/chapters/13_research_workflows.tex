...
\ChapterMeta{
This chapter describes end-to-end research loops and the supporting helpers for subsets, sweeps, and benchmarks.
}{
It creates repeatable experiments with an artifact-first discipline so results can be compared and regressed reliably.
}{
It assumes you are using stable JSON artifacts and, when applicable, protocol presets; keep subsets deterministic to make comparisons fair.
}
...
\begin{itemize}
  \item required: \cmd{base_cmd}, and either \cmd{param_grid} or \cmd{param_list}
  \item optional: \cmd{run_dir}, \cmd{metrics.path}, \cmd{metrics.keys}
  \item optional outputs: \cmd{result_jsonl}, \cmd{result_csv}, \cmd{result_md}
