...
\ChapterMeta{
Describe end-to-end research loops (export \(\rightarrow\) validate \(\rightarrow\) evaluate \(\rightarrow\) compare) and the supporting helpers (subsets, sweeps, benchmarks).
}{
Creates repeatable experiments with artifact-first discipline so results can be compared and regressed reliably.
}{
Requires using stable JSON artifacts and (when applicable) protocol presets; use deterministic subsets to keep comparisons fair.
}
