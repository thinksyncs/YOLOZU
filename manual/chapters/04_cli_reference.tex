...
\ChapterMeta{
Summarize the high-traffic commands and show how to discover the full surface via \cmd{--help} on both CLIs.
}{
Reduces time-to-first-run and clarifies which commands are install-safe vs repo-only power tools.
}{
Use \cmd{yolozu --help} (pip CLI) or \cmd{python3 tools/yolozu.py --help} (repo wrapper); details change over time so prefer \cmd{--help} as the source of truth.
}
