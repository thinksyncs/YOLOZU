...
\ChapterMeta{
This chapter summarizes the most common commands and shows how to discover the full CLI surface via \cmd{--help}.
}{
It reduces time to first run and clarifies which commands are install-safe versus repo-only.
}{
Use \cmd{yolozu --help} for the pip CLI or \cmd{python3 tools/yolozu.py --help} for the repo wrapper; details change over time so \cmd{--help} is the source of truth.
}
...
\path{rtdetr_pose.train_minimal}, and \cmd{yolozu test} converts YAML keys to
\path{yolozu.scenarios_cli} arguments.
...
Backbone swaps are controlled by model config JSON (not by changing adapter commands directly):
\begin{lstlisting}[language=bash]
yolozu train configs/examples/train_setting.yaml \
  --model-config rtdetr_pose/configs/base.json
\end{lstlisting}

For backbone contract and supported names, see \path{docs/backbones.md}.
...
\noindent This mirrors the documented real-model flow in \path{docs/real_model_interface.md}.
...
For a comprehensive list, consult \path{tools/manifest.json} (machine-readable) and the docs index (human-readable).
