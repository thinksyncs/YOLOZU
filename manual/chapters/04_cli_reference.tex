...
\midrule
Train (default) & \path{configs/examples/train\_setting.yaml} & \cmd{yolozu train configs/examples/train\_setting.yaml} \\
...
Resume & same as train contract & \cmd{yolozu train configs/examples/train\_contract.yaml --run-id exp01 --resume} \\
Test (scenario) & \path{configs/examples/test\_setting.yaml} & \cmd{yolozu test configs/examples/test\_setting.yaml --adapter precomputed --predictions reports/predictions.json} \\
\bottomrule
\end{longtable}
