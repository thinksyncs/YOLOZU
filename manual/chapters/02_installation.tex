...
\ChapterMeta{
Set up a working \yolozu{} environment (pip install for users, editable install for repo workflows) and verify with quick sanity checks.
}{
Produces a reproducible toolchain and makes backend availability explicit (CPU-only vs GPU-capable paths).
}{
Requires Python 3.10+; optional extras are backend-specific (Torch/ONNX Runtime/COCO). TensorRT workflows typically require Linux + NVIDIA.
}
