...
\begin{itemize}
  ...
  \item \cmd{sidecar}: depth is read from sidecar metadata (\cmd{depth\_path}/\cmd{depth}) and tracked via \cmd{depth\_valid}.
  \item \cmd{fuse\_mid}: depth is fused after projector (outside backbone \([P3,P4,P5]\) swap boundary).
\end{itemize}
