# TTT Integration Plan (MIM primary, Tent optional)
...
- Support two methods: **MIM** (primary) and **Tent** (optional).
- Provide clear logging (`updated_param_count`, loss summaries) and deterministic runs via seed.
...
- **yolozu/tta/config.py** (optional new)
...
- `build_loader(records, *, batch_size, shuffle=False, max_batches=None)`
...
- `--ttt` (bool): enable TTT
- `--ttt-method {mim,tent}` (default: mim)
- `--ttt-steps` (int, default: 1)
- `--ttt-lr` (float, default: 1e-4)
- `--ttt-batch-size` (int, default: 4)
...
- Tent-specific: `--ttt-entropy-weight` (optional), `--ttt-logits-key` (optional)
...
- Add `ttt` block under meta when `--wrap` is used:
  ...
  - `updated_param_count`, `mask_ratio` (MIM), `loss_summary`.
